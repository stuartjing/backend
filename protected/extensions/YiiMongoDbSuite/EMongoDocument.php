...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
	...
	public function getCollection()
	{
		...
		*/
		################## 文档名称支持trueCollection变量 #####################
		$cName = ($this->getCollectionName()=='entity'&&$this->trueCollection)?$this->trueCollection:$this->getCollectionName();
		if(!isset(self::$_collections[$cName])){	//没有链接对象则新建
			self::$_collections[$cName] = $this->getDb()->selectCollection($cName);
		}
	...
	public function find($criteria=null)
	{
		...
		if($this->beforeFind())
		{
			...
			#### 获取当前真实表名并加入结果集 ######
			$doc['tC'] = $this->getCollection()->getName();
			################ 20130304 ############
	...
	public static function model($className=__CLASS__)
	{
		...
		*/
		############# 查询模型兼容传入表名称 ############
		$classIndex = $className;					//表名默认与class同名
		$classNameArr = explode('_', $className);
		//若有'_'分隔，则前半部分大写首字母作为模型类名，整体为动态模型
		if(count($classNameArr)>1){
			$className = ucfirst($classNameArr[0]);
		}
		
		if(isset(self::$_models[$classIndex])){
			return self::$_models[$classIndex];
		}else{
			if(strtolower($className)=='entity' && $classIndex!=$className){
				$model=self::$_models[$classIndex]=new $className($classIndex);
			}else{
				$model=self::$_models[$classIndex]=new $className();
			}
			$model->attachBehaviors($model->behaviors());
			return $model;
		}
