...
abstract class EMongoSoftDocument extends EMongoDocument
{
	...
	protected function instantiate($attributes)
	{
		...
		*/
		############# 获取已存在实体模型 ##############
		if(isset($attributes['tC'])){
			$classNameArr = explode('_', $attributes['tC']);
			if(strtolower($classNameArr['0'])=='entity'){
				$model = new $class($attributes['tC'], null);	//创建第二个参数来传递操作类型
			}else{
				$model = new $class(null);
			}
			unset($attributes['tC']);	//清理该属性
		}else{
			$model = new $class(null);
		}
		...
		$model->initSoftAttributes(	//注册动态属性
			array_diff(
